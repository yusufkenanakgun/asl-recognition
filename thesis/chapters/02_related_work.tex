...
static signs. For word-level recognition, however, they must be
combined with explicit temporal modelling, because motion, handshape
transitions, and two-handed coordination jointly carry the meaning of
a sign. The next two
...
cheaper to evaluate at inference time, since their input
dimensionality is bounded by a handful of keypoints rather than a full
video tensor. Even so, the published landmark-based pipelines,
including SPOTER~\cite{bohacek2022spoter}, report accuracy on desktop
hardware rather than latency, energy, or model-size figures on a
mobile device. The recent survey of Rastgoo
...
throughput alongside Top-1 accuracy on WLASL-100.

Recent landmark-only work has explicitly targeted edge devices on the
same WLASL-100 subset addressed here. Singh
et~al.~\cite{singh2026pbrc} combine MediaPipe hand landmarks with a
parallel bidirectional reservoir computing (PBRC) architecture and
report a Top-1 accuracy of \SI{60.85}{\percent} together with CPU-side
training and inference times on a workstation, positioning the
pipeline as suitable for edge deployment without measuring it on a
mobile device. Parallel activity at the letter task is visible in the
dual-pipeline Portuguese Sign Language application of Fonseca
et~al.~\cite{fonseca2025lgp}, which contrasts a TensorFlow~Lite
MobileNetV2 against a MediaPipe+MLP back end (the latter served from a
remote endpoint), and in the MobileNet+TFLite Indian Sign Language
pipeline of Balamurugan et~al.~\cite{balamurugan2025isl}. The
smartphone-collected ASL fingerspelling dataset
FSboard~\cite{georg2024fsboard} further documents that the
smartphone-deployment regime is now treated as a first-class research
target rather than a niche concern. Relative to this body of work,
the contribution of the present chapter is the end-to-end on-device
characterisation of four classifiers (image and landmark $\times$
letter and word) under a single Android deployment, with throughput
and latency reported on both a budget and a flagship handset and a
single-user out-of-distribution gap quantified on each cell; the
following section locates the resulting design in that landscape.
...
methods on this subset, namely the cross-domain TK-3D ConvNet of Li
...
accuracy. Relative to the contemporaneous landmark-only PBRC of Singh
et~al.~\cite{singh2026pbrc}, the modelling difference is recurrence
(a two-layer BiLSTM) versus a parallel echo-state reservoir; the
evaluation difference is the on-device characterisation on Android
budget and flagship handsets, which neither PBRC nor any other
landmark-only WLASL-100 method published to date reports. The resulting contribution is therefore not a new accuracy record
on WLASL-100 -- the present model trails the closest published
analogue (PBRC, hands-only MediaPipe) by roughly five points and
the pose-transformer SPOTER by roughly seven, on the same
canonical person-mixed split -- but a demonstration that a
landmark-only, mobile-deployable pipeline can be evaluated on
that same benchmark under the same protocol with its accuracy,
footprint and on-device latency all reported jointly, where the
within-family methods that lead the accuracy column report only
