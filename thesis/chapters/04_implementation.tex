...
backbone with its ImageNet weights, and Google's
MediaPipe~Hands~0.10.32~\cite{zhang2020mediapipe} handles hand-landmark
extraction. OpenCV~4.13 is used to decode video files and to
...
Figure~\ref{fig:impl-asl-samples} reports a small sample of training
images drawn from the eight classes that exemplify the range of
handshapes the classifier has to discriminate.

\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{figures/ch4_asl_alphabet_samples.png}
\caption{Representative samples from the ASL Alphabet test partition:
...
Figure~\ref{fig:impl-wlasl-drink} and
Figure~\ref{fig:impl-wlasl-mother} illustrate the input that the
...
isolated sign. The two examples were chosen to span the
one-handed and two-handed cases that dominate WLASL-100.

\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{figures/ch4_wlasl_strip_drink.png}
\caption{Six evenly-spaced frames from a WLASL clip of the
one-handed sign \emph{drink}. The dominant hand starts at rest
and travels to the mouth in a single motion, which the BiLSTM
sees as a short trajectory in landmark space.}
\label{fig:impl-wlasl-drink}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{figures/ch4_wlasl_strip_mother.png}
\caption{Six evenly-spaced frames from a WLASL clip of the
two-handed sign \emph{mother}. Both hands are active and the
dominant hand makes thumb contact with the chin; the
two-channel landmark representation of
Section~\ref{sec:impl-landmark} is what allows the BiLSTM
to use both hands jointly.}
...
ratio in its family on commodity hardware: \num{5.3} million parameters in
its stock ImageNet configuration (reduced to \num{4.1} million once the
...
The architecture is a two-layer bidirectional~\cite{schuster1997bidirectional}
LSTM~\cite{hochreiter1997lstm} with a hidden width of
...
input to the currently selected one. The high-level data flow and the
per-frame sequence diagram are presented in
...
\begin{itemize}
  ...
  \item \textbf{BiLSTM single} maintains a 32-frame sliding buffer of
        ...
        Section~\ref{sec:impl-landmark}. The Words analyzer wraps this
        buffer in a small state machine designed for manual reading of
        a single isolated sign rather than continuous classification.
        When MediaPipe first reports a hand, a 30-frame ($\approx$1~s)
        \textsf{Get~Ready} countdown is displayed as a full-screen
        overlay; this countdown resets if the hand leaves the frame
        before it completes, so that the buffer is never primed with a
        half-formed pose. The 32-frame buffer then fills while a
        sequence-progress overlay is shown. Once the buffer is full
        the classifier is invoked \emph{once}: if the top-1 confidence
        exceeds a small dead-zone threshold (0.10) the prediction is
        latched on screen and further classifier calls are suppressed,
        so that the displayed Top-3 panel does not drift while the
        user reads it. The latch releases automatically when MediaPipe
        reports an empty frame for eight consecutive analyzer
        callbacks ($\approx$270~ms), at which point the buffer is
        cleared, the countdown re-arms, and a new sign is accepted.
        Predictions below the dead-zone threshold do not latch: the
        sliding buffer continues to advance and the next full window
        is re-evaluated, which is the recovery path when the buffer
        spans a transition between two signs.
...
The figures below illustrate the deployed application on the two
test phones. All screenshots were captured directly from the
running APK on the devices listed in
Section~\ref{sec:results-on-device-protocol}; no post-processing
beyond cropping the device's status-bar area was applied.

Figure~\ref{fig:ui-entry} shows the application's two entry
surfaces: the Home screen, which navigates to the Letters and
Words tabs, and the permission gate that handles a denied camera
permission. Figures~\ref{fig:ui-letters-tecno}
and~\ref{fig:ui-letters-xiaomi} show the Letters tab on the
entry-level TECNO and the flagship Xiaomi respectively;
representative letters are displayed rather than the full alphabet
because the layout is identical across classes.
Figure~\ref{fig:ui-words-drink} shows the three states of the Words
tab single-shot lock state machine
(Section~\ref{sec:impl-android-pipeline}) on the WLASL sign
\texttt{drink}: the \textsf{Get Ready} countdown, the
sequence-progress overlay during buffer fill, and the latched
...
\end{subfigure}
\caption{Entry-surface screens: Home (navigation to Letters and
...
\caption{space}
\end{subfigure}
\caption{Letters tab on the entry-level TECNO SPARK 20C. The
top-right badge reports live FPS and end-to-end latency
(Section~\ref{sec:results-on-device-letters}); the bottom panel
displays the Top-3 prediction together with each class's
confidence.}
...
\caption{X}
\end{subfigure}
\caption{Letters tab on the flagship Xiaomi 15T Pro. The on-screen
layout is identical to the TECNO build; the visual differences
between the two figures (sharper preview, more consistent colour)
are attributable to the device's image-signal pipeline, which is
also the most likely source of the on-device accuracy gap reported
in Section~\ref{sec:results-qualitative-letters}.}
...
\end{subfigure}
\caption{The three states of the Words tab single-shot lock state
machine on the WLASL sign \texttt{drink}, captured on the Xiaomi
15T Pro. (a) MediaPipe first reports a hand and the 30-frame
\textsf{Get Ready} countdown is shown; (b) the 32-frame
sliding buffer is filling and a sequence-progress overlay is
displayed; (c) the buffer is full, the classifier has been invoked
once and the top-1 prediction is latched on screen until the
hand-down release fires.}
...
\url{https://github.com/yusufkenanakgun/asl-recognition}.} Training the
...
converted to TensorFlow~Lite by \texttt{src/convert\_to\_tflite.py};
the converted files live in \texttt{models/tflite/} for direct
consumption by the Android project. Conversion is deterministic, so
the mobile-side results in Chapter~\ref{ch:test-results} match the
offline numbers exactly.
