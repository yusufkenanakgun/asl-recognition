...
This chapter reports the empirical evaluation of the four offline
classifiers introduced in Chapter~\ref{ch:implementation} together
with the on-device behavior of the Android demonstration application.
The aim is twofold. First, to verify that each model meets the
quantitative acceptance bars adopted for this thesis: a Top-1
accuracy of at least \SI{40}{\percent} on the WLASL-100
word-recognition task and a confident letter classifier for the ASL
Alphabet. The project proposal specifies the evaluation metrics
(Top-1, Top-5, per-class F1, confusion matrix, FPS, model size in
MB) but does not pin numeric pass/fail thresholds; the bars used here
are therefore self-imposed yardsticks adopted by this thesis, with
their rationale stated in
Section~\ref{sec:results-words-acceptance}. Second, to make the
deployability claim of this thesis quantitative by comparing the four
classifiers along axes that matter on a phone: accuracy, model size
and inference latency.

The chapter is organised as two complementary evaluation layers.
Sections~\ref{sec:results-letters}--\ref{sec:results-errors} report
the \emph{offline} evaluation: classifier accuracy and error
structure on the held-out test partitions, with results positioned
against published baselines on the same data. Sections
\ref{sec:results-on-device} and \ref{sec:results-qualitative} report
the \emph{deployment} evaluation: on-device throughput and latency,
followed by a single-user manual test that bounds the
offline-to-on-device accuracy gap. The two layers are reported in
this order because the second is meaningful only conditional on the
first; the manual evaluation in particular is a supplementary
deployability check that the proposal does not require but that the
deployability framing of this thesis benefits from.
...
Before reporting the offline numbers, the conditions under which they
were measured should be made explicit. The ASL Alphabet test partition
is a random split of a single-source Kaggle dataset whose images were
collected by one contributor in a controlled indoor setting with a
uniform background and a fixed camera distance. The same person,
lighting, and background therefore appear in both the training and
the test partitions, and the offline Top-1 figures reported in this
section should be read as a sanity check that the training pipeline
converges and that the exported checkpoints preserve accuracy --- not
as a capability measure for unseen signers, rooms, or cameras. The
on-device manual evaluation in
Section~\ref{sec:results-qualitative-letters} is where the actual
capability signal is sought.

With that caveat stated, both letter classifiers reach near-ceiling
accuracy on the ASL Alphabet test partition. EfficientNet-B0 produces
\emph{zero} misclassifications on the 13\,050 test images and therefore
reaches a Top-1 accuracy of exactly 100\,\%, with a per-class F1 score
of 1.000 across all 29 classes. The landmark MLP reaches 99.38\,\%
Top-1 accuracy on the subset of test images for which MediaPipe
successfully detected a hand (73.09\,\% of the test partition, as
discussed in Section~\ref{sec:impl-mlp}). The two numbers therefore
measure two different effective populations: the image classifier
evaluates on the complete test set, while the landmark classifier
evaluates on the detector-recoverable subset and is undefined on the
remaining \SI{26.91}{\percent} of images. To place the two classifiers
on a common footing, Table~\ref{tab:results-letters} reports a
coverage-adjusted Top-1 that recomputes the landmark MLP's accuracy on
the deployment-relevant population --- the 12\,600 non-\texttt{nothing}
test images, since neither classifier processes the \texttt{nothing}
class in the deployed pipeline (Section~\ref{sec:results-qualitative-letters})
--- with detector failures counted as misclassifications. Under this
fair-comparison framing the MLP's effective accuracy is
\SI{75.23}{\percent} against EfficientNet's \SI{100}{\percent}; the
\SI{24.77}{percentage point} gap is the operationally honest cost of
the MediaPipe coverage gap when the two classifiers are framed as
drop-in alternatives rather than as complementary stages.
...
Coverage-adjusted Top-1\textsuperscript{a} & 100.00\,\% & 75.23\,\% \\
...
\end{tabular}
\\[0.4em]
\footnotesize\textsuperscript{a}Top-1 recomputed on the non-\texttt{nothing} subset of the test partition (12\,600 images) with detector failures counted as misclassifications, matching the deployment-relevant population on which both classifiers run.
\end{table}

The training trajectories of the two classifiers are shown in
Figure~\ref{fig:results-lc-cnn} and Figure~\ref{fig:results-lc-mlp};
both converge rapidly and trigger the early-stopping criterion well
before the configured epoch budget, with no train--val divergence
pattern that would suggest overfitting.
...
\caption[Training trajectory of the EfficientNet-B0 letter classifier.]{Training trajectory of the EfficientNet-B0 letter
...
\caption[Training trajectory of the landmark MLP letter classifier.]{Training trajectory of the landmark MLP letter
...
\caption[Row-normalised confusion matrix of the EfficientNet-B0 letter classifier.]{Row-normalised confusion matrix of the EfficientNet-B0
...
\caption[Row-normalised confusion matrix of the landmark MLP letter classifier.]{Row-normalised confusion matrix of the landmark MLP letter
...
A third observation closes the loop on the dataset caveat stated at
the top of this section: because both classifiers were trained and
evaluated on a controlled, single-contributor split, neither the
\SI{100}{\percent} nor the \SI{99.38}{\percent} figure should be
treated as a prediction of real-world performance. The on-device
manual evaluation in Section~\ref{sec:results-on-device} reports the
substantially noisier behavior of both classifiers under the natural
lighting and background variation of a smartphone camera; the gap
between the offline test accuracy and the on-device accuracy is the
appropriate measure of how well each model would generalize in
production use.
...
Before reporting the numbers, the scientific status of what is being
measured here should be made explicit. The 201-clip test partition is the
canonical \emph{person-mixed} WLASL-100 split: the same signer can appear
in both the training and the test partitions, and this is the protocol
under which every entry of the literature comparison in
Table~\ref{tab:results-wlasl-lit} reports its number. The offline Top-1
figures in this section should therefore be read as a
\emph{protocol-comparable benchmark point} on that canonical split,
i.e.\ a like-for-like comparison axis against the published baselines,
rather than as a signer-independent generalization estimate. A clean
generalization estimate would require a signer-disjoint evaluation that
the public WLASL-100 release does not directly support and that the
present thesis does not construct. What this chapter \emph{does} report
is therefore bracketed: an inflated-by-design offline benchmark number
under the canonical protocol
(Sections~\ref{sec:results-words}--\ref{sec:results-errors}) and a
known-sign on-device upper bound under naturalistic camera conditions
(Section~\ref{sec:results-qualitative-words}). The band between the two
-- the actual production-conditions accuracy on a signer the system has
never seen -- is left explicitly unmeasured and is revisited as the
headline limitation in Section~\ref{sec:conclusion-limitations}.

\begin{table}[h]
\centering
\caption[Per-seed and ensemble accuracy on the WLASL-100 test set.]{Per-seed and ensemble accuracy on the WLASL-100 test set.
Top-1 cells include the 95\,\% Wilson binomial confidence interval
computed on the 201-clip test partition; at this sample size a single
misclassification moves Top-1 by \SI{0.50}{percentage point} and the
CI spans roughly \(\pm 7\) points around each estimate.}
\label{tab:results-words-seeds}
\begin{tabular}{lccc}
\toprule
\textbf{Model} & \textbf{Top-1 (\%) [95\,\% CI]} & \textbf{Top-5 (\%)} & \textbf{Macro F1} \\
\midrule
BiLSTM seed 0           & 53.23 [46.3, 60.0] & 80.60 & 0.491 \\
BiLSTM seed 1           & 52.74 [45.9, 59.5] & 78.11 & 0.495 \\
BiLSTM seed 2           & 56.22 [49.3, 62.9] & 79.10 & 0.524 \\
BiLSTM seed 3           & 49.75 [42.9, 56.6] & 77.61 & 0.447 \\
BiLSTM seed 4           & 48.76 [41.9, 55.6] & 79.60 & 0.469 \\
\midrule
Five-seed mean          & 52.14 & 79.00 & 0.485 \\
Five-seed std (range)   & 2.97 ($\pm$3.73) & $\pm$1.49 & $\pm$0.038 \\
\midrule
\textbf{Ensemble (logit avg.)} & \textbf{56.22 [49.3, 62.9]} & \textbf{81.09} & \textbf{0.521} \\
\bottomrule
\end{tabular}
\end{table}
...
\caption[Training trajectories of the five BiLSTM seeds.]{Training trajectories of the five BiLSTM seeds trained
under the final configuration. Thin grey lines are individual seeds,
...
rather than alarming. The 95\,\% Wilson binomial confidence interval
at this sample size spans roughly \(\pm 7\) points around each Top-1
estimate (Table~\ref{tab:results-words-seeds}), so the per-seed CIs all
overlap heavily and a difference of a few points between any two
models on this test set is, in isolation, indistinguishable from
sampling noise. This is the appropriate prior against which the
ensemble result has to be read.

The ensemble lifts the five-seed mean from 52.14\,\% to 56.22\,\% on
Top-1 and the five-seed mean from 79.00\,\% to 81.09\,\% on Top-5, in
line with the variance-reduction expectation for averaging
decorrelated predictors~\cite{dietterich2000ensemble}. To check
whether the +4.08-point Top-1 lift over the per-seed mean exceeds the
inherent noise of a 201-clip evaluation, a paired clip-level bootstrap
(10\,000 resamples, paired by clip index; same RNG seed for every
comparison) was run on the per-seed and ensemble correctness vectors;
the implementation is documented in
\texttt{src/faz6/stat\_analysis.py} and its output in
\texttt{models/faz6\_v2/stat\_analysis.json}. Against the per-clip
empirical seed mean the ensemble's Top-1 lift is
\(+4.09\) points with a 95\,\% bootstrap CI of \([+0.80, +7.46]\)
(\(p = 0.015\)), so the gain over the mean of an unselected seed is
statistically distinguishable from zero on this test set. Against the
best single seed (seed 2), however, the same procedure returns no
difference: bootstrap 95\,\% CI \([-4.48, +4.98]\) (\(p \approx 1.0\)),
and McNemar's exact test on the paired clip-level predictions returns
\(p = 1.0000\) with \(b = c = 12\) (the ensemble repairs 12 clips on
which seed~2 is wrong while introducing 12 new errors of its own; the
net effect on Top-1 is exactly zero, which is why both reach 113/201
correct). The operationally honest reading is therefore that the
ensemble's offline contribution on this 201-clip test set is to
\emph{remove the seed-selection gamble} --- a deployer who picks an
unknown seed expects 52.14\,\% \(\pm\) 3 points, the ensemble pins the
result at 56.22\,\% --- rather than to exceed what a validation-best
single seed could have delivered. The case for the ensemble at
deployment time rests on this variance reduction and on the on-device
manual test of Section~\ref{sec:results-qualitative-words}, where the
\(+11\)-point gap between single and ensemble on 100 trials carries
its own statistical reading.

\subsection{Comparison with the Adopted Acceptance Bars}
\label{sec:results-words-acceptance}

The project proposal does not fix a numeric pass/fail threshold for
the word task; it lists the metrics on which the system is to be
judged (Top-1, Top-5, F1, confusion, FPS, MB) and leaves the
acceptance bar to be set within the thesis. The bars adopted here
are two: a hard acceptance criterion of Top-1 $\geq 40\,\%$ and a
stretch target of Top-1 $\geq 60\,\%$ on the WLASL-100 test
partition. The hard bar is the rounded floor below which a hands-only
landmark model would not be defensible against the published
WLASL-100 baselines (Pose-TGCN at \SI{55.43}{\percent} and I3D at
\SI{65.89}{\percent}, Section~\ref{sec:related-temporal}); the
stretch target sits midway between Pose-TGCN and I3D and is the bar
at which a landmark model would be qualitatively competitive with
the original RGB benchmark. The ensemble result of
56.22\,\% clears the hard bar by 16.22 percentage points and reaches
within 3.78 percentage points of the stretch target. The
...
against published results on the same WLASL-100 split. Every row of the
table reports its number on the canonical person-mixed protocol of
Li et al.~\cite{li2020wlasl}, which is the only WLASL-100 protocol that
all of these methods share; the table is therefore a comparability axis
on that protocol rather than a ranking of signer-independent
generalization, and the absolute level of each entry, including the
56.22\,\% reported here, should be read with that qualification in
mind. The comparison is further approximate because the entries
differ in input modality, preprocessing pipeline and hardware budget;
nevertheless, the table communicates the relative position of a
hands-only landmark model among the available approaches.

\begin{table}[h]
\centering
\caption[Reported Top-1 accuracy on the WLASL-100 test split (literature comparison).]{Reported Top-1 accuracy on the WLASL-100 test split, under
the dataset's canonical person-mixed protocol shared by all entries.
Numbers are quoted from each publication; the modality column
distinguishes RGB, body pose, body+hand pose and hand-only inputs.
The table is a comparability axis on the canonical protocol, not a
ranking of signer-independent generalization.}
...
2026 & PBRC~\cite{singh2026pbrc}           & Hands only (MediaPipe) & 60.85 \\
\midrule
...
The accuracy gap relative to the literature is real and is owned
explicitly here. Against the RGB-based pipelines the hands-only
configuration sacrifices roughly 10 points of Top-1 against the
single-stream I3D baseline and just over 20 points against the
strongest reported method on WLASL-100, namely the cross-domain
TK-3D ConvNet of Li et al. that augments I3D with knowledge
transferred from subtitled news videos; this is the expected cost
of dropping a raw-video 3D convolutional backbone. The more
informative comparison is within the landmark-only family, where
the present result is also \emph{not} the accuracy leader: the
ensemble lands one point above the body-pose Pose-TGCN baseline
but \SI{4.63}{} points below the contemporaneous PBRC pipeline of
Singh et al.~\cite{singh2026pbrc} (which combines MediaPipe hand
landmarks with parallel bidirectional reservoir computing) and
\SI{6.96}{} points below the transformer-based SPOTER, despite
using a smaller architecture and a strictly smaller input feature
set (21 hand landmarks per frame, against 55 upper-body keypoints
for Pose-TGCN and 54 head-body-hand landmarks for SPOTER).

The thesis-level claim on the word task is therefore deliberately
not an accuracy claim. The contribution that distinguishes this
work from PBRC and SPOTER is the same-protocol footprint and
latency characterisation that those papers do not provide. PBRC
is the closest published analogue in input modality (hand
landmarks via MediaPipe) and stated deployment target (edge
devices), yet reports CPU-side training and inference time on a
workstation rather than measurements on a deployed mobile
artefact; SPOTER reports desktop-side numbers only. Neither
paper reports a model-size figure in megabytes against its
Top-1 entry, neither paper reports an on-device end-to-end
latency on a smartphone, and neither paper reports an
offline-to-on-device accuracy gap on a known-sign single-user
probe. The present chapter reports all three
(Sections~\ref{sec:results-on-device} and
\ref{sec:results-qualitative}) on two distinct Android devices.
The trade-off characterized in Chapter~\ref{ch:implementation}
therefore plays out as expected on the offline benchmark:
heavier RGB or multi-stream pipelines win on accuracy, lighter
landmark pipelines win on deployability. The present work is the
first landmark-only entry in this table to report on-device
latency and model footprint jointly with offline accuracy under
...
the proposal's real-time inference requirement (NFR-01) on the offline
machine. The corresponding on-device numbers,
...
was trained with single-hand 63-dimensional input on a 1013-video
Kaggle bundle of WLASL-100, without sequence-level augmentation and
without ensemble averaging; it reached only 20.00\,\% validation
accuracy on the same WLASL-100 split. The remaining 40 percentage
points to the ensemble's 60.08\,\% were gained incrementally over
nine modifications, grouped into three methodological pillars: (i)
\emph{data engineering}, the yt-dlp-based recovery of link-rotted
WLASL videos described in Section~\ref{sec:impl-data}; (ii) the
\emph{extraction pipeline} that introduced WLASL frame-window
cropping and two-hand landmarks with presence flags; and (iii) the
\emph{training-recipe refinements} that layer sequence augmentation,
mixup, a weighted sampler, hidden-width capacity and five-seed
ensembling on top of the new feature stream. Each modification was
ablated by training three independently seeded models on the same
WLASL-100 split as the final configuration, and the resulting mean
validation accuracies are reported in
Table~\ref{tab:results-ablation}. This table is the single most
useful artefact for understanding which design choices carried the
most weight on this dataset.

\begin{table}[h]
\centering
\caption[Forward-additive ablation from the 20.00\,\% baseline to the 60.08\,\% ensemble.]{Forward-additive ablation from the initial 20.00\,\% baseline to
the final 60.08\,\% ensemble. Each row introduces a single new
component on top of the previous row and reports the mean validation
Top-1 over three independently seeded training runs (mean~$\pm$~half-range);
the final ensemble row is the published five-seed logit-averaging model.
The three component groups (data, extraction, training) are demarcated
horizontally.}
...
\midrule
0 & Initial baseline (1013 vids, single hand, no aug.)  & 20.00 & -- \\
\midrule
\multicolumn{4}{l}{\emph{Data engineering}} \\
1 & + yt-dlp recovery (1013\,$\to$\,1465 videos)        & 36.08 $\pm$ 2.05 & +16.08 \\
\midrule
\multicolumn{4}{l}{\emph{Extraction pipeline}} \\
2 & + two-hand extraction, frame-window crop, presence flags & 46.37 $\pm$ 1.18 & +10.29 \\
\midrule
\multicolumn{4}{l}{\emph{Training-recipe refinements}} \\
3 & + light sequence augmentation\textsuperscript{a}    & 48.97 $\pm$ 0.34 & +2.60 \\
4 & + stronger frame dropout ($p{=}0.15$)               & 49.93 $\pm$ 0.20 & +0.96 \\
5 & + time-scale $\pm 20\,\%$                           & 50.89 $\pm$ 0.97 & +0.96 \\
6 & + mixup ($\alpha = 0.2$)                            & 53.09 $\pm$ 0.34 & +2.20 \\
7 & + hidden size 128 $\rightarrow$ 192                 & 51.85 $\pm$ 1.43 & \textbf{$-$1.24} \\
8 & + \texttt{WeightedRandomSampler} \& mixup $\alpha {=} 0.4$ & 52.54 $\pm$ 3.66 & +0.69 \\
9 & Five-seed ensemble (logit averaging)                & \textbf{60.08} & +7.54 \\
\midrule
\multicolumn{2}{l}{\textbf{Test set (final ensemble)}}    & \multicolumn{2}{c}{\textbf{56.22 / 81.09 (Top-1 / Top-5)}} \\
\bottomrule
\end{tabular}}
\\[0.4em]
{\footnotesize\textsuperscript{a}Temporal jitter $\pm 4$ frames, frame dropout $p{=}0.05$, Gaussian noise $\sigma{=}0.02$.}
\end{table}

Three observations stand out. First, the largest single contribution to the
40-point trajectory is the data engineering step. The yt-dlp recovery of
448 link-rotted videos (Section~\ref{sec:impl-data}), holding every other
hyperparameter and the feature representation fixed at their V1 values,
adds 16.08 validation points on its own. This is more than every
training-side change combined and underlines the dataset-size sensitivity
of this WLASL-100 setup: under \num{1013} videos the recurrent classifier
sits at chance plus a memorization band, whereas with \num{1465} videos the
same architecture clears 36\,\%. Second, the extraction-pipeline upgrade -- two-hand
landmarks with presence flags, plus cropping to the WLASL
\texttt{frame\_start}/\texttt{frame\_end} window -- contributes a further
10.29 points without touching the training recipe at all. Together, the
two non-training pillars explain 26.4 of the 40 points, and the recipe
refinements account for the remaining 14. Third, the per-component effects
within the training pillar are small (\SIrange{0.7}{2.6}{} points each)
and several of them target specific failure modes rather than driving the
headline number on their own. The \texttt{WeightedRandomSampler} (step~8)
re-balances the loss against the long tail of WLASL-100, where the
majority of words have between one and four training clips; without it
the model collapses onto the head classes and the macro F1 score drops
noticeably even when Top-1 stays competitive. The increase in mixup
intensity from $\alpha = 0.2$ to $\alpha = 0.4$ closes the remaining
single-seed gap to the adopted stretch target without changing the
validation curve shape.

\subsection{Interaction Effect: Hidden Width}
\label{sec:results-ablation-interaction}

Step~7 of Table~\ref{tab:results-ablation} -- doubling the hidden width
from 128 to 192 on top of the previous configuration -- is the only
row with a negative $\Delta$ in isolation ($-1.24$ points). At face value
this would argue against keeping the wider hidden state in the deployed
model. A complementary run, however, retrained the
\emph{full} final configuration (steps 1--8 of
Table~\ref{tab:results-ablation}: data, extraction, full augmentation
stack, mixup $\alpha = 0.4$, weighted sampler) with hidden width \emph{fixed}
at 128 and reached only 49.39\,\% validation Top-1 and 49.75\,\% test
Top-1, against the 52.54\,\% validation and 56.22\,\% test obtained
with hidden 192. Holding the rest of the recipe fixed, removing
hidden 192 from the final configuration costs $-3.15$ validation
points and $-6.47$ test points. The wider hidden state therefore
underperforms when added on top of a partially regularized recipe
($-1.24$ in isolation) but contributes a $+4.39$-point
\emph{interaction effect} once the full regularization stack is in
place ($+3.15 - (-1.24)$), motivating its joint adoption with mixup
$\alpha = 0.4$ and the weighted sampler rather than as a standalone
capacity argument. Hidden 192 is kept in the deployed model because,
conditional on the rest of the recipe, removing it is the most
expensive of the single-component changes the ablation surfaced.
This complementary run was a single retraining of the full recipe
with hidden 128, not the three-seed protocol applied to steps~1--8;
the $+4.39$-point interaction magnitude should be read with that
caveat, as the available ablation budget did not extend to a
multi-seed re-evaluation of the wider hidden state under the final
recipe.

\subsection{Ensemble (Step~9)}
\label{sec:results-ablation-ensemble}

Step~9 reports the published five-seed logit-averaging ensemble at
60.08\,\% validation Top-1. The +7.54-point gap from step~8 conflates
the ensemble-versus-best-single-seed effect with a sample-pool effect
(three-seed ablation mean vs.\ five-seed pool), which the ablation
budget cannot fully separate; the validation-best single seed in the
published pool is at 58.85\,\%, so the ensemble-attributable
component is bounded at $+1.23$ validation points. On the held-out
test partition the same five-seed pool yields the variance-reduction
reading of the ensemble already detailed in
Section~\ref{sec:results-words} and not repeated here. The additional
cost is purely at inference time and is bounded by the five-fold
latency penalty quantified in Section~\ref{sec:results-on-device}.
...
\caption[Row-normalised confusion matrix of the five-seed BiLSTM ensemble.]{Row-normalised confusion matrix of the five-seed BiLSTM
...
\emph{zero} examples in the test partition used here because the
source video URLs for their official test instances (four for
\texttt{book}, two for \texttt{later}) were no longer accessible at
download time; the remaining train and validation clips were obtained
successfully (\texttt{book}: 11~train, 2~val; \texttt{later}:
11~train, 4~val). The macro F1 score of 0.521 reported in
...
over the full 100. The ensemble does evaluate these two glosses on the
validation partition, where it reaches 1/2~Top-1 on \texttt{book} and
2/4~Top-1 on \texttt{later}, both below the \SI{60.08}{\percent}
validation Top-1 averaged across all 100 classes. The model has
therefore learned both glosses but only weakly, with two of the four
\texttt{later} validation clips collapsing onto \texttt{short} and
\texttt{computer} and the failing \texttt{book} clip being predicted
as \texttt{white}; the ground-truth label for the failing \texttt{book}
clip does not appear in the Top-5 either. The absence of these classes
from the offline Top-1 figure is thus a property of WLASL-100 source
attrition rather than of the test split policy, and the validation
numbers establish that the model would not have substantially raised
the Top-1 score had the missing test clips been available. Second, the Top-5 accuracy of 81.09\,\%
...
inside the proposal's non-functional budget (NFR-01, \SI{10}{FPS}
...
\begin{table}[h]
\centering
\caption[On-device throughput and latency of the two word classifiers.]{On-device throughput and latency of the two word
...
purpose of the experiment is not to produce a new accuracy number
that would compete with the offline results, because a single human
signer cannot sample the variation of the underlying populations.
Instead, it is to verify that the proposal's qualitative criterion
(``the recognised letter or word is written to the screen'') is
satisfied in real use, and to characterise where the offline numbers
are misleading.
...
Because the WLASL-100 vocabulary is not part of the author's active
sign repertoire, each recorded Words trial was preceded by a short
familiarisation step: the author reviewed the reference video for
the gloss and rehearsed the sign until satisfied with its
production. A single trial was then captured, selected by the author
\emph{not by the classifier output} as a best-effort production.
This is the standard \emph{familiarisation plus best-effort trial}
formulation used when evaluating motor-skill systems with a
non-expert user: it is intended to separate classifier accuracy from
the user's own learning curve, and approximates a ``user knows the
sign'' condition. The cost of this design choice is that the
reported manual-test accuracy is an upper bound under the known-sign
assumption; a naive signer making first-attempt productions would
obtain a substantially lower number, and the same caveat is repeated
in Section~\ref{sec:conclusion-limitations}. A further sub-source of
variance applies specifically to the Words trials: a small number of
WLASL-100 glosses have more than one valid sign production, and for
these classes the author selected one variant arbitrarily before
familiarisation. Since each gloss is tested with a single trial, the
reported per-gloss outcome does not isolate classifier accuracy from
the model's sensitivity to which variant was produced, and this
contributes to the single-user out-of-distribution variance discussed
in Section~\ref{sec:conclusion-limitations}. Letter trials did not
require a familiarisation step, because the canonical fingerspelling
poses of the ASL Alphabet are part of the author's prior practice.
...
cells. This is consistent with the chapter's claim that
the letter classifiers are deployment-ready, under the single-user
known-sign conditions of the manual test.
...
\SI{7}{\percent} points (ensemble). This is the order of magnitude
anticipated when this thesis adopted its manual-test acceptance
bars: \SI{35}{\percent} for the single model and
\SI{40}{\percent} for the ensemble, both chosen to allow roughly a
\SIrange{10}{15}{\percent}-point OOD penalty relative to the offline
targets of Section~\ref{sec:results-words-acceptance}. The manual
test itself is not part of the proposal evaluation plan; it is
included as a supplementary deployability check whose bars are this
thesis's own. At $n = 100$ the 95\,\% Wilson binomial CIs are
$[29.1, 47.8]$ for the single model and $[39.4, 58.7]$ for the
ensemble: a one-sided exact binomial test against the adopted bars
returns $p = 0.298$ for the single model (\SI{38.0}{\percent} vs
\SI{35}{\percent}) and $p = 0.042$ for the ensemble
(\SI{49.0}{\percent} vs \SI{40}{\percent}), so the ensemble clears
its bar at the 5\,\% level whereas the single model's lead over its
bar is within sampling noise on the 100-trial sample. Both numbers
should be read as bounded by this sample size rather than as
distribution-level capability claims; a larger and multi-signer
manual evaluation is listed as future work in
...
thresholds that the manual-test plan of this thesis initially
considered for the Words task. The interpretation is methodological rather than a
...
delivers the deployment-level outcome called for by the proposal:
the recognised letter or word appears on screen, the application
reports its own FPS and latency, and the gap between offline and
on-device accuracy is bounded and explainable. Read jointly with the
offline evaluation of Sections~\ref{sec:results-letters}--\ref{sec:results-errors},
the two layers converge on a consistent picture: the four
classifiers meet the adopted offline acceptance bars on the
held-out test partitions
(Section~\ref{sec:results-words-acceptance}), and the deployed system
preserves that behaviour under realistic camera and on-device
inference conditions to the extent that hardware capacity permits.
Letters cleared the adopted on-device acceptance bar by a wide
margin on both phones; Words cleared the adopted on-device bar
(Top-1~$\geq$~\SI{35}{\percent} single,
$\geq$~\SI{40}{\percent} ensemble) on the Xiaomi and were not
measurable on the TECNO for the reasons discussed in
Section~\ref{sec:results-on-device-words}. These measurements expose
two limitations of the delivered system --- the asymmetric latency
cost of CPU-only execution on entry-level hardware and the
single-user OOD penalty observed on EfficientNet under the TECNO's
camera pipeline --- both taken up in Chapter~\ref{ch:conclusion}.
